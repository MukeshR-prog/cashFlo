      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          <CardContent className="p-6">
            ...
            <h3 className="text-3xl font-bold mb-2">${totalUncollected.toLocaleString()}</h3>
            <p className="text-xs text-muted-foreground">Contracted but not yet converted into cash.</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-red-950/20 border-red-900/40 ring-1 ring-red-500/20">
          <CardContent className="p-6">
            ...
            <p className="text-xs text-red-500/80">Immediate runway impact and higher collection risk.</p>
          </CardContent>
        </Card>
              ...
              <tbody>
                {invoices.map((invoice, i) => (
                  <tr key={i} className={`border-b border-border hover:bg-muted/30 transition-colors ${invoice.status === 'paid' ? 'opacity-60 bg-muted/20' : ''}`}>
                    ...
                    <td className="px-4 py-4 text-muted-foreground">{invoice.issueDate}</td>
                    <td className="px-4 py-4 text-muted-foreground">{invoice.dueDate}</td>
                    <td className="px-4 py-4 font-medium text-right">${invoice.amount.toLocaleString()}</td>
